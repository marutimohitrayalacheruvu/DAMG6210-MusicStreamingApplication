...
DROP USER JOHNDOE;
SELECT * FROM user_tables WHERE table_name='ARTIST';
SELECT * FROM user_tables WHERE table_name IN ('ALBUM','SONGS');
------------------------------------roleArtist----------------------------------------------------
DECLARE
  ncount NUMBER;
BEGIN
  ...
  ELSE
    EXECUTE IMMEDIATE 'CREATE ROLE ARTIST_ROLE';
       EXECUTE IMMEDIATE 'GRANT EXECUTE ON APP_ADMIN.ARTIST_PKG TO ARTIST_ROLE';
...
--    EXECUTE IMMEDIATE 'GRANT EXECUTE ON APP_ADMIN.INSERT_ALBUM_ARTIST_M TO ARTIST_ROLE'; 
    ...
    END LOOP;
    COMMIT;
    FOR x IN (SELECT * FROM user_tables WHERE table_name IN ('FAVOURITE', 'GENRE','ARTIST'))
    LOOP
...
--   EXECUTE IMMEDIATE 'GRANT EXECUTE ON APP_ADMIN.DELETE_PLAYLIST TO USER_ROLE';
   ...
   EXECUTE IMMEDIATE 'GRANT EXECUTE ON APP_ADMIN.USER_PKG TO USER_ROLE';
   FOR x IN (SELECT * FROM user_tables where table_name in ('ARTIST','ALBUM','SONGS', 'FAVOURITE', 'PLAYLIST', 'USERS','SONGS_PLAYLIST'))
   LOOP
...
BEGIN
    ...
    ELSE
        ...
        app_admin.insert_pkg.insert_artist('Taylor', 'Swift', 'A talented musician', 'TAYLOR_SWIFT');
...
BEGIN
  ...
  IF nCount > 0 THEN 
    ...
    ELSE
        ...
        insert_pkg.insert_or_update_user('John', 'Doe', 'johndoe@example.com', 'JOHNDOE', 'active', SYSDATE);
