...
BEGIN
  ...
  ELSE
    ...
    EXECUTE IMMEDIATE 'GRANT EXECUTE ON APP_ADMIN.INSERT_ALBUM TO ARTIST_ROLE';
...
BEGIN
 ...
 if (ncount>0) THEN
  dbms_output.put_line('USER_ROLE ALREADY EXISTS');
   ...
   FOR x IN (SELECT * FROM user_tables where table_name in ('ARTIST','ALBUM','SONGS', 'FAVOURITE', 'PLAYLIST'))
   LOOP
